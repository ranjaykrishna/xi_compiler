...
public class Generate {
	...
	public static Instruction generateTile(NameIR name, TempTable tTable){
		...
		//System.out.println("generate name ir gen:"+t);
		return inst;
	}
	...
	public static Instruction generateTile(CompUnitIR comp, TempTable tTable){
		...
		ArrayList<String> DV = comp.getListOfDVS();
		for(String poop : DV) {
			result.add(new Tile(0, poop, null, null));
		}
		ArrayList<String> size = comp.getSizeSetup();
		for(String poop : size) {
			result.add(new Tile(0, poop, null, null));
		}
		return result;
	}
	...
	public static ExpressionInstruction generateTileLabel(LabelIR label, TempTable tTable) {
		ExpressionInstruction inst = new ExpressionInstruction();
		inst.setAddr(new AddressingMode(label.getName(), "rip",true));
		label.setLEA(true);
		return inst;
	}
	
	public static ExpressionInstruction generateTileLabelNonLEA(LabelIR label, TempTable tTable) {
		ExpressionInstruction inst = new ExpressionInstruction();
		label.setLEA(false);
		inst.setAddr(new AddressingMode(label.getName(), "rip",true));
		return inst;
	...
	public static Instruction generateTile(MoveIR move, TempTable tTable){
		Instruction inst = new Instruction();
		Instruction childSrc;
		Instruction childDest = generateTile((SyntaxIR) move.getChildren().get(0),tTable);
		if(move.getChildren().get(1) instanceof LabelIR) {
			childSrc = generateTileLabel((LabelIR) move.getChildren().get(1), tTable);
		} else {
			childSrc = generateTile((SyntaxIR) move.getChildren().get(1),tTable);
		}
		...
		if (addr1.getMemAccess() && addr2.getMemAccess()){
		...
		else if (addr2.getMemAccess() && !(move.getChildren().get(1) instanceof LabelIR)){
			...
			iii.add(new Tile(1,"popq",new AddressingMode("r13"),null));
			iii.add(new Tile(1,"movq",new AddressingMode("r13"),addr2));
			return iii;
		}else if (addr1.getMemAccess()){
			Instruction iii = new Instruction();
			iii.addAll(childSrc.getTiles());
			iii.add(new Tile(1,"pushq",addr1,null));
			iii.addAll(childDest.getTiles());
		...
		if(move.getChildren().get(1) instanceof LabelIR && ((LabelIR) move.getChildren().get(1)).getLEA())
			inst.add(new Tile(1,"leaq",addr1,addr2));
		else
	...
	public static ExpressionInstruction generateTile(TempIR temp, TempTable tTable){
		...
		if(id.startsWith("_ret")) {
			...
			offset = 1*(offset)*8;
			inst.setAddr(new AddressingMode(offset, "rdi"));                                             		
		} 
		else if(!(letter.equals("t"))) {
			if(id.startsWith("_args0"))
				inst.setAddr(new AddressingMode("rdi"));
			else if(id.startsWith("_args1"))
				inst.setAddr(new AddressingMode("rsi"));
			...
			else if(id.startsWith("_args4"))
				inst.setAddr(new AddressingMode("r8"));
			else if(id.startsWith("_args5"))
				inst.setAddr(new AddressingMode("r9"));
			else if(id.startsWith("_args")){
				//System.out.println("setting offset to"+(Integer.parseInt(id.substring(5,id.length())) - 5) * 8);
			...
			if(id.startsWith("_args0")) {
				...
				inst.setAddr(new AddressingMode((Integer.parseInt(id.substring(5,id.length())) - 4) * 8, "rbp"));			
			} 
			else {
	...
	public static Instruction generateTile(CallIR call, TempTable tTable){
		...
		//inst.add(new Tile(0, "calling_" + call.prettyPrint()+":", null, null));
	...
	private static Instruction handleArguments(CallIR call, TempTable tTable) {
		...
		for(int i = 1; i < call.getChildren().size(); i++){
			if(call.getChildren().get(i) instanceof LabelIR)
				argumentTileLists.add(generateTileLabelNonLEA((LabelIR) call.getChildren().get(i), tTable));
			else
		...
		if(retsize > 1) {
			CallIR c = new CallIR(new NameIR("_I_alloc_i"), new ConstIR(8*(retsize)));
			c.setretsize(1);
			ExpressionInstruction ex = (ExpressionInstruction) generateTile(c,tTable);
			inst.addAll(ex.getTiles());
			inst.add(new Tile(1, "movq", ex.getAddr(), new AddressingMode("rdi")));
			inst.add(new Tile(1, "movq", new AddressingMode(retsize), new AddressingMode(0,"rdi")));
			inst.add(new Tile(1, "add", new AddressingMode(8), new AddressingMode("rdi")));
		}
		...
		case 2:
			if(retsize > 1) {
				...
				inst.add(new Tile(1,"popq",addrRSI,null));
			}
		case 1:
			if(retsize > 1) {
				inst.add(new Tile(1,"popq",addrRSI,null));
			} else {
				inst.add(new Tile(1,"popq",addrRDI,null));
			}
		case 0:
			//Do nothing
			break;
	...
	private static Instruction makeCall(CallIR call, TempTable tTable) {
		...
		AddressingMode func;
		/*
		 * This checks if we're calling on a memir or a nameir
		 */
		//TODO: fix this so it works
		//this should use the stuff that prof mentioned on piazza
		if(!(call.getChildren().get(0) instanceof NameIR)) {
			Instruction callName = generateTile((SyntaxIR) call.getChildren().get(0), tTable);
			//System.out.println("call name generate:"+((ExpressionInstruction) callName).getAddr());
			func = ((ExpressionInstruction) generateTile((SyntaxIR) call.getChildren().get(0), tTable)).getAddr();
			func = func.makeMultiply(0);
			//System.out.println("func gen:"+func);
		} else
			func = new AddressingMode(((NameIR)call.getChildren().get(0)).getLabel(), true);
		...
		*///assuming the ret call moves the stack pointer back to before the arguments we should 
		...
		if(call.getretsize() > 1 && argumentsize > 5) {
			for(int i = argumentsize; i > 5; i --) {
				inst.add(new Tile(1,"popq",new AddressingMode("r12"),null));
			}
		} else if (argumentsize > 6) {
			for(int i = argumentsize; i > 6; i --) {
				inst.add(new Tile(1,"popq",new AddressingMode("r12"),null));
			}
		}
		return inst;
	...
	public static ExpressionInstruction generateTile(MemIR mem, TempTable tTable){
		...
		if (addr.getMemAccess()){
			inst.add(new Tile(1,"movq", addr, new AddressingMode("r14")));
			inst.setAddr(new AddressingMode(true,"r14"));
		}
		else{
